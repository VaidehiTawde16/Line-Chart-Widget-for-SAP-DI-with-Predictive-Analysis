...
const addOneDayToISO = (dateString) => {
  const date = new Date(dateString);
...
const string2Array = (inputString) => {
  const regex = /\[(.*?)\]/g;
  const result = [];
  let match;
  ...
  class LineSamplePrepped extends HTMLElement {
    ...
    checkLevel5 = (obj) => {
      ...
      if (found) {
        ...
        let nextMonthDetails = getMonthdetails(label);
